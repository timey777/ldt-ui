...
class OpenGLDrawerAndroid::ImplData {
public:
...
static void gl_apply_scissor(OpenGLDrawerAndroid::ImplData* d, bool enable, int x, int y, int w, int h);
static void gl_apply_clip_rect(OpenGLDrawerAndroid::ImplData* d, bool hasClip, const render::Rect& clip);
static void gl_bind_program(OpenGLDrawerAndroid::ImplData* d, unsigned int prog);
static void gl_bind_vao(OpenGLDrawerAndroid::ImplData* d, unsigned int vao);
static void gl_bind_array_buffer(OpenGLDrawerAndroid::ImplData* d, unsigned int buf);
static void gl_bind_tex0(OpenGLDrawerAndroid::ImplData* d, unsigned int tex);

static void rect_batch_flush(OpenGLDrawerAndroid::ImplData* d);

static void ensure_textured_vbo_capacity(OpenGLDrawerAndroid::ImplData* d, size_t requiredFloatCount) {
	if (!d) return;
...
static void ensure_rect_vbo_capacity(OpenGLDrawerAndroid::ImplData* d, size_t requiredFloatCount) {
	if (!d) return;
...
static void textured_batch_reset(OpenGLDrawerAndroid::ImplData* d) {
	if (!d) return;
	d->texBatch.kind = OpenGLDrawerAndroid::ImplData::TexBatchKind::None;
	d->texBatch.tex = 0;
...
static void textured_batch_flush(OpenGLDrawerAndroid::ImplData* d);
...
static void textured_batch_note_flush(OpenGLDrawerAndroid::ImplData* d, TexBatchFlushReason reason) {
	if (!d) return;
	auto& b = d->texBatch;
	bool hadWork = (b.kind != OpenGLDrawerAndroid::ImplData::TexBatchKind::None && b.tex != 0 && !b.verts.empty());
	if (!hadWork) return;
...
static void textured_batch_flush_with_reason(OpenGLDrawerAndroid::ImplData* d, TexBatchFlushReason reason) {
...
static void textured_batch_flush(OpenGLDrawerAndroid::ImplData* d) {
	if (!d) return;
	auto& b = d->texBatch;
	if (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::None || b.tex == 0 || b.verts.empty()) {
	...
	unsigned int prog = (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Text) ? d->progTexText : d->progTex;
	int uSampler = (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Text) ? d->uTexSamplerText : d->uTexSampler;
	int uColor = (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Text) ? d->uTexColorText : d->uTexColor;
	...
	if (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Text) {
		++d->stat_bufferSubData_text;
	} else if (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Image) {
...
static void textured_batch_begin(OpenGLDrawerAndroid::ImplData* d,
	OpenGLDrawerAndroid::ImplData::TexBatchKind kind,
	unsigned int tex,
	const float color[4]) {
	if (!d) return;
	auto& b = d->texBatch;
	...
	if (b.kind != OpenGLDrawerAndroid::ImplData::TexBatchKind::None && !keyMatch) {
	...
	if (b.kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::None) {
		b.kind = kind;
		b.tex = tex;
		b.color[0] = color[0];
		b.color[1] = color[1];
...
static void textured_batch_append_quad(OpenGLDrawerAndroid::ImplData* d,
	OpenGLDrawerAndroid::ImplData::TexBatchKind kind,
	unsigned int tex,
	const float color[4],
	...
	if (kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Text) {
		++b.textGlyphs;
	} else if (kind == OpenGLDrawerAndroid::ImplData::TexBatchKind::Image) {
		++b.imageQuads;
	}
}

static void rect_batch_reset(OpenGLDrawerAndroid::ImplData* d) {
	if (!d) return;
	auto& b = d->rectBatch;
	b.active = false;
	b.hasClip = false;
...
static void rect_batch_begin(OpenGLDrawerAndroid::ImplData* d) {
	if (!d) return;
	auto& b = d->rectBatch;
	bool clipMatch = (b.hasClip == d->hasClip) && (!b.hasClip || rect_exact_equal(b.clip, d->clip));
...
static void rect_batch_append(OpenGLDrawerAndroid::ImplData* d,
	float x, float y, float w, float h,
...
static OpenGLDrawerAndroid::ImplData::FontAtlasPage createFontAtlasPage(int w, int h) {
	OpenGLDrawerAndroid::ImplData::FontAtlasPage page;
	page.width = w;
	page.height = h;
	page.penX = 1;
	page.penY = 1;
...
static void gl_apply_sdf_clip(OpenGLDrawerAndroid::ImplData* d, unsigned int prog,
	...
	glUniform1f(uSdfClipRadiusLoc, cornerRadius * d->scaleX);
}
...
static void gl_apply_clip(OpenGLDrawerAndroid::ImplData* d) {
	if (!d) return;
...
OpenGLDrawerAndroid::OpenGLDrawerAndroid() { d = new ImplData(); }
OpenGLDrawerAndroid::~OpenGLDrawerAndroid() { shutdown(); delete d; d = nullptr; }
...
OpenGLDrawerAndroid::OpenGLDrawerAndroid() {}
OpenGLDrawerAndroid::~OpenGLDrawerAndroid() {}
void OpenGLDrawerAndroid::init(int, int) {}
void OpenGLDrawerAndroid::resize(const ldt::FramebufferSizePx&, const ldt::ContentScale&) {}
void OpenGLDrawerAndroid::clear(float, float, float, float) {}
void OpenGLDrawerAndroid::drawRect(float, float, float, float, const DrawStyle&) {}
void OpenGLDrawerAndroid::drawRoundedRect(float, float, float, float, float, float, const DrawStyle&) {}
void OpenGLDrawerAndroid::drawImage(ImageHandle, float, float, float, float, const DrawStyle&) {}
void OpenGLDrawerAndroid::drawText(const std::string&, float, float, const Font&, const DrawStyle&, float, const TextLayoutParams&) {}
TextMetrics OpenGLDrawerAndroid::measureText(const std::u32string&, const FontDesc&, float) { return TextMetrics(); }
TextMetrics OpenGLDrawerAndroid::measureText(const std::string&, const FontDesc&, float) { return TextMetrics(); }
float OpenGLDrawerAndroid::getLineHeight(const FontDesc&) { return 0.0f; }
ImageHandle OpenGLDrawerAndroid::createImageFromMemory(const unsigned char*, int, int, int) { return (ImageHandle)0; }
void OpenGLDrawerAndroid::unloadImage(ImageHandle) {}
void OpenGLDrawerAndroid::setClipRect(const render::Rect&) {}
bool OpenGLDrawerAndroid::intersectClipRect(const render::Rect&, render::Rect*) { return false; }
bool OpenGLDrawerAndroid::getClipRect(render::Rect*) const { return false; }
void OpenGLDrawerAndroid::resetClip() {}
void OpenGLDrawerAndroid::setClipCornerRadius(float) {}
float OpenGLDrawerAndroid::getClipCornerRadius() const { return 0.0f; }
void OpenGLDrawerAndroid::pushTransform(float, float) {}
void OpenGLDrawerAndroid::popTransform() {}
void OpenGLDrawerAndroid::present() {}
void OpenGLDrawerAndroid::shutdown() {}

#endif // LDT_ENABLE_RENDERING

#if defined(LDT_ENABLE_RENDERING)

void OpenGLDrawerAndroid::init(int width, int height) {
...
void OpenGLDrawerAndroid::resize(const ldt::FramebufferSizePx& framebufferSize, const ldt::ContentScale& contentScale) {
...
void OpenGLDrawerAndroid::clear(float r, float g, float b, float a) {
	// Reset GL state that affects clearing (scissor/FBO/viewport) before glClear.
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glDisable(GL_SCISSOR_TEST);
	// Android surfaces may be recreated independently from higher-level render state.
	// Reassert the viewport before clearing so a stale viewport cannot blank the frame.
	...
	d->gl.scissorW = -1;
	d->gl.scissorH = -1;
	// Reset SDF clip state
	d->clipCornerRadius = 0.0f;
}

void OpenGLDrawerAndroid::drawRect(float x, float y, float w, float h, const DrawStyle& style) {
	x += d->offsetX;
	y += d->offsetY;
...
void OpenGLDrawerAndroid::drawRoundedRect(float x, float y, float w, float h, float rx, float ry, const DrawStyle& style) {
	x += d->offsetX;
	y += d->offsetY;
	// preserve ordering: flush any pending textured batch before rect batching
...
void OpenGLDrawerAndroid::drawImage(ImageHandle img, float x, float y, float w, float h, const DrawStyle& style) {
	x += d->offsetX;
	y += d->offsetY;
...
static void ensureGlyph(OpenGLDrawerAndroid::ImplData* d, const OpenGLDrawerAndroid::ImplData::FontKey& key, uint32_t c) {
	// load face if needed
	...
	if (pixelW > 0 && pixelH > 0 && g->bitmap.buffer) {
		...
		auto tryPack = [&](OpenGLDrawerAndroid::ImplData::FontAtlasPage& page) -> bool {
			// new row if needed
...
void OpenGLDrawerAndroid::drawText(const std::string& text, float x, float y, const Font& font, const DrawStyle& style, float maxWidth, const TextLayoutParams& layout) {
	x += d->offsetX;
	y += d->offsetY;
	// preserve ordering: flush any pending rect batch before textured batching
	rect_batch_flush(d);
...
TextMetrics OpenGLDrawerAndroid::measureText(const std::u32string& text, const FontDesc& fontDesc, float wrapWidth) {
	return measureText(utf32_to_utf8(text), fontDesc, wrapWidth);
}

TextMetrics OpenGLDrawerAndroid::measureText(const std::string& utf8text, const FontDesc& fontDesc, float wrapWidth) {
	TextMetrics m;
	// map FontDesc -> Font
	Font f;
	...
	for (char32_t ch : u32text) {
		...
		float advance = (float)(face->glyph->advance.x >> 6) / d->scaleX;

		if (wrapWidth > 0.0f && curW + advance > wrapWidth) {
			if (curW > maxW) maxW = curW;
			curW = advance;
			lines++;
		} else {
...
float OpenGLDrawerAndroid::getLineHeight(const FontDesc& fontDesc) {
...
ImageHandle OpenGLDrawerAndroid::createImageFromMemory(const unsigned char* data, int width, int height, int channels) {
...
void OpenGLDrawerAndroid::unloadImage(ImageHandle img) {
	if (img != 0) {
...
// clip helpers
void OpenGLDrawerAndroid::setClipRect(const render::Rect& r) {
...
bool OpenGLDrawerAndroid::intersectClipRect(const render::Rect& r, render::Rect* outNewClip) {
...
bool OpenGLDrawerAndroid::getClipRect(render::Rect* outCurrent) const {
...
void OpenGLDrawerAndroid::setClipCornerRadius(float radius) {
	if (!d) return;
...
float OpenGLDrawerAndroid::getClipCornerRadius() const {
	return d ? d->clipCornerRadius : 0.0f;
}

void OpenGLDrawerAndroid::pushTransform(float dx, float dy) {
...
std::shared_ptr<ITextLayout> OpenGLDrawerAndroid::createTextLayout(const std::string& text, const Font& font, const TextLayoutParams& layout, float maxWidth) {
	...
	// Metrics
	float lineHeight = (layout.lineHeight > 0.0f) ? layout.lineHeight : font.size * 1.2f;
...
void OpenGLDrawerAndroid::drawTextLayout(const std::shared_ptr<ITextLayout>& layout, float x, float y, const DrawStyle& style) {
